...
export async function setOAuthStateCookie(res: VercelResponse, state: string) {
    res.setHeader('Set-Cookie', `${OAUTH_STATE_COOKIE}=${encodeURIComponent(state)}; ${baseCookieAttrs(600)}`);
}
...
export async function setSessionCookie(res: VercelResponse, payload: SessionPayload) {
    ...
    res.setHeader('Set-Cookie', `${SESSION_COOKIE}=${encodeURIComponent(token)}; ${baseCookieAttrs(60 * 60 * 24 * 7)}`);
}
