...
export function RequestDetailPage() {
    ...
    const mediaControls = isMediaTeam(item.createdByEmail) || true; // set to true for mock; tighten later
    ...
    return (
        <div className="mx-auto w-full max-w-4xl px-6 py-10">
            ...
            <div className="mt-6 grid gap-4">
                ...
                <div className="rounded-xl border border-border bg-card p-4">
                    <div className="text-xs font-semibold text-muted-foreground">Deliverable</div>
                ...
                Folder: {item.deliverableFolderId}
                        ...
                        <div className="mt-2 text-sm text-muted-foreground">Not delivered yet.</div>
                    )}
                </div>

                {/* Media team controls (mock) */}
                {mediaControls && (
                    <div className="rounded-xl border border-border bg-card p-4">
                        <div className="text-xs font-semibold text-muted-foreground">Media actions</div>

                        <div className="mt-3 grid gap-3">
                            <div>
                                <div className="text-xs text-muted-foreground">Deliverable folder id</div>
                                <input
                                    className="mt-2 h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                                    placeholder="Drive folderId (later)"
                            ...
                            {upd.isError && (
                                <div className="text-sm text-muted-foreground">Update failed.</div>
                            )}
                        </div>
                    </div>
                )}
