...
export function useMediaSearch(q: MediaSearchQuery, opts: Options = {}) {
    ...
    const queryKey = ['media', mode, pageSize, q.folderId, q.text, q.type, q.sort, q.tags.join(',')] as const;
    ...
    >({
        ...
        getNextPageParam: (lastPage) => {
            const next = mode === 'recursive' ? lastPage.nextCursor : lastPage.nextPageToken;
            return next ?? undefined;
        },
    });

    const items: MediaItem[] = (r.data?.pages ?? []).flatMap((p) => p.items ?? []);
